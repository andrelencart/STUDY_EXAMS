...
int count_neib(t_life life, int h, int w){
	...
	if (h - 1 >= 0 && life.map[h - 1][w] == '0')
		neib++;
	if ((w + 1 < life.x && h + 1 < life.y) && life.map[h + 1][w + 1] == '0')
		neib++;
	if ((w - 1 >= 0 && h - 1 >= 0) && life.map[h - 1][w - 1] == '0')
		neib++;
	if ((w - 1 >= 0 && h + 1 < life.y) && life.map[h + 1][w - 1] == '0')
		neib++;
	if ((w + 1 < life.x && h - 1 >= 0) && life.map[h - 1][w + 1] == '0')
		neib++;
	return neib++;
}
